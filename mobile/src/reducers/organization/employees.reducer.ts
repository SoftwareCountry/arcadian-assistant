        ...
        case 'LOAD_EMPLOYEE_FINISHED':
            ...
            if (oldEmployee !== undefined) {
                ...
                if (oldEmployee.departmentId !== newEmployee.departmentId) {
                   employeeIdsByDepartment.update(oldEmployee.departmentId, Set(), oldColelction => oldColelction.remove(oldEmployee.employeeId));
                }
            }

            employeeIdsByDepartment = employeeIdsByDepartment.update(newEmployee.departmentId, Set(), oldCollection => oldCollection.add(newEmployee.employeeId));
