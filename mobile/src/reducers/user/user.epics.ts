...
export const updateUserPreferencesEpic$ = (action$: ActionsObservable<UpdateUserPreferences>, appState: AppState, deps: DependenciesContainer) =>
    action$.ofType('UPDATE-USER-PREFERENCES').pipe(
        switchMap(action => {
            ...
            return deps.apiClient.put(`/user-preferences/`, requestBody, { 'Content-Type': 'application/json' })
                .pipe(handleHttpErrors(false), map(() => loadUserPreferencesFinished(action.preferences))
                );
        }));
